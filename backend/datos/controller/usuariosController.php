<?php
  ...
  $accion = $_REQUEST['accion'];
	...
	$data = array("Nombre"=>$_REQUEST['txtName'],"Cedula"=>$_REQUEST['txtDoc'], "Direccion"=>$_REQUEST['txtDir'],
	        "Telefono"=>$_REQUEST['txtTel'], "Email"=>$_REQUEST['txtEml'], "Perfil"=>$_REQUEST['txtPerfil'],
    ...
    case "upd":
      $jsondata = array();
      $vimg = $_FILES['txtImgup']['name'];
      if ($vimg != "") {
      	// si file viene lleno
      	$data = array("Nombre"=>$_REQUEST['txtNameup'],"Cedula"=>$_REQUEST['txtDocup'], "Direccion"=>$_REQUEST['txtDirup'],
      	        "Telefono"=>$_REQUEST['txtTelup'], "Email"=>$_REQUEST['txtEmlup'], "Perfil"=>$_REQUEST['txtPerfilup'],
      	        "Usuario"=>$_REQUEST['txtUserup'], "Status"=>1, "Updated_by"=>$vname,  "Updated_at"=>date('Y-m-d H:i:s')
      	      );
      	...
      	if(($vType == "png") or ($vType == "jpg")){
            //validamos el peso de la imagen
            if($_FILES['txtImgup']['size'] <= 1000000){
              //validamos las dimensiones de la imagen
              $infoImagen = getimagesize($_FILES['txtImgup']['tmp_name']);
              if ($infoImagen[0] <= 225 || $infoImagen[1] <= 225) {

                $carpeta = "../public/usuarios/".$_REQUEST['txtIdup'];
                $destino2 = "../public/usuarios/".$_REQUEST['txtIdup']."/".$vimg;
                $dest = "public/usuarios/".$_REQUEST['txtIdup']."/".$vimg."'-";
                $ruta2 = $_FILES['txtImgup']['tmp_name'];
